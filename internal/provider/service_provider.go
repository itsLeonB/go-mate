...
func ProvideServices(configs *config.Config, repositories *Repositories, utils *Utils) *Services {
	scoringService := service.NewScoringServiceNaive()

	return &Services{
		Auth:           service.NewAuthService(repositories.User, utils.Hash, utils.JWT),
		Recommendation: service.NewRecommendationServiceNaive(repositories.User, scoringService),
	}
}
